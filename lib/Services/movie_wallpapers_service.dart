...
class MovieWallpapersService {
  ...
  Stream<List<Movie>> getAllMovies() async* {
    ...
    yield rawMovieData.map((m) => Movie.fromMap(m)).toList();
  }

  Stream<List<MovieWallpaper>> getMovieWall(Movie m) async* {
    ...
    yield rawMovieData.map((m) => MovieWallpaper.fromMap(m)).toList();
  }
